...
import { downloadPLOTemplate } from "../utils/excelHelper";
import { showSuccess, showError, showInfo } from "../utils/toast";

const PLOBulkUploadLayer = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await ploService.bulkImport(rawFile);
      setUploadResult(res); setDone(true);
      if (res?.status?.code === 0) showSuccess(res?.status?.message || "PLOs uploaded successfully");
  ...
  return (
    <div className="card h-100 p-0 radius-12">
      <div className="card-header border-bottom py-16 px-24 d-flex align-items-center justify-content-between">
        <h5 className="mb-0">Bulk Upload PLOs</h5>
        <button onClick={() => navigate("/plos")} className="btn btn-sm btn-outline-secondary radius-8">
      ...
      <div className="card-body p-24">
        ...
        <div className="mb-32">
          ...
          <p className="text-secondary-light text-sm ms-40 mb-4">
            Download <strong>PLOs_Template.csv</strong> and fill in your PLO data. Required columns:
          </p>
          <ul className="text-secondary-light text-sm ms-40 mb-12">
            <li><strong>program_code</strong> — Program code (e.g., BSCS)</li>
            <li><strong>plo_number</strong> — PLO number (e.g., 1, 2, 3…)</li>
            <li><strong>description</strong> — Full PLO description text</li>
          </ul>
          <button onClick={downloadPLOTemplate} className="btn btn-outline-primary radius-8 d-inline-flex align-items-center gap-8 ms-40">
            <Icon icon="vscode-icons:file-type-excel" className="text-xl" /> Download PLOs_Template.csv
          </button>
        </div>
            ...
            <div className="ms-40 d-flex gap-12">
              <button className="btn btn-primary radius-8 px-32 py-11" onClick={handleUpload} disabled={uploading}>
                {uploading
                  ...
                  : <><Icon icon="solar:upload-bold" className="me-6" />Upload PLOs</>}
              </button>
        ...
        {done && uploadResult && (
          <div className="ms-40">
            ...
            {resultRows.length > 0 && (
              <div className="table-responsive mb-16" style={{ maxHeight: 300, overflowY: "auto" }}>
                <table className="table bordered-table mb-0">
                  <thead>
                    <tr><th>#</th><th>Program</th><th>PLO No.</th><th>Status</th><th>Message</th></tr>
                  </thead>
                  <tbody>
                    {resultRows.map((r, i) => (
                      <tr key={i}>
            ...
            <div className="d-flex gap-12">
              <button className="btn btn-primary radius-8 px-32 py-11" onClick={() => navigate("/plos")}>Go to PLO List</button>
...
export default PLOBulkUploadLayer;